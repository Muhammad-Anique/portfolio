...
import {
  Phone, Globe, MessageSquare, Mic, Headphones, Zap,
  Shield, BrainCircuit, PhoneIncoming, Server,
} from "lucide-react";
...
export default function SofiaProposal() {
  ...
  function doLogin() {
    if (username === "sofia" && password === "sofia123456") {
      setLoggedIn(true);
      setError(false);
    } else {
      setError(true);
            ...
            <div className="sec" id="exec">
              ...
              <div className="card card-accent-amber">
                ...
                <table className="tbl">
                  ...
                  <tbody>
                    <tr><td><span className="badge badge-red">L1</span></td><td>Doctor</td><td>Supreme authority. Overrides everything. No audit required.</td></tr>
...
PAGO → ESTORNADO (refund by doctor)`}</div>
            ...
            {/* ═══ 11.5. VOICE AGENT ═══ */}
            <div className="sec" id="voice">
              <div className="sec-label">Section 11.5</div>
              <h2 className="sec-title">Sofia Voice Agent — Phone &amp; WhatsApp</h2>
              <div className="alert green">
                <strong>Beyond Text: Sofia Speaks.</strong> Sofia is available as a fully conversational voice agent — reachable via international phone numbers and WhatsApp voice calls. Powered by VAPI (voice AI), Telnyx (global telephony), and Twilio (phone + WhatsApp Business), patients simply call in and Sofia handles scheduling, reminders, and inquiries with natural conversation — no IVR menus, no hold music.
              </div>

              <div className="g3">
                <div className="tech-card" style={{ borderTopColor: "#7c3aed" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>
                    <BrainCircuit size={22} color="#7c3aed" />
                    <div className="tech-name">VAPI</div>
                  </div>
                  <div className="tech-role" style={{ color: "#7c3aed" }}>Conversational AI Layer</div>
                  <div className="tech-desc">Real-time voice AI platform. Converts speech → text, routes through Sofia&apos;s LLM pipeline, responds with natural TTS. Handles interruptions, turn-taking, and latency invisibly.</div>
                  <div style={{ marginTop: 8 }}><span className="badge badge-slate">Cloud SaaS</span></div>
                </div>
                <div className="tech-card" style={{ borderTopColor: "#0891b2" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>
                    <Globe size={22} color="#0891b2" />
                    <div className="tech-name">Telnyx</div>
                  </div>
                  <div className="tech-role" style={{ color: "#0891b2" }}>International Phone Numbers</div>
                  <div className="tech-desc">SIP trunking and DID numbers across 140+ countries. Ultra-low-latency media routing. Provides the Brazilian +55 number (or any international DID) that patients actually dial.</div>
                  <div style={{ marginTop: 8 }}><span className="badge badge-slate">Global Telco</span></div>
                </div>
                <div className="tech-card" style={{ borderTopColor: "#e11d48" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>
                    <Phone size={22} color="#e11d48" />
                    <div className="tech-name">Twilio</div>
                  </div>
                  <div className="tech-role" style={{ color: "#e11d48" }}>Phone + WhatsApp Business</div>
                  <div className="tech-desc">Programmable voice and WhatsApp Business API. Patients can call Sofia&apos;s dedicated WhatsApp number or receive post-call SMS confirmations — all on one platform.</div>
                  <div style={{ marginTop: 8 }}><span className="badge badge-slate">Cloud Comms</span></div>
                </div>
              </div>

              <h3 className="sub">Voice Architecture</h3>
              <div className="codeblock">{`Patient dials international number  →  Telnyx DID / Twilio number
                                              │
                                              ▼
                                   VAPI Voice Server
                               (real-time STT/TTS, WebSocket)
                                              │
                                              ▼
                            Sofia LLM 3-Tier (DeepSeek → OpenAI → Claude)
                                              │
                                              ▼
                          n8n Orchestration  ─  same 5 workflows
                          (INGEST / CLASSIFY / DECIDE / RESPOND / ERROR)
                                              │
                                              ▼
                               Zoho CRM  ─  same Pipeline E1-E6
                          ┌─────────────────────────────────────────┐
                          │  Sisclinic booking  │  PIX link via SMS  │
                          │  WhatsApp follow-up │  Correlation ID    │
                          └─────────────────────────────────────────┘`}</div>

              <div className="g2">
                <div className="card card-accent">
                  <div className="card-title blue" style={{ display: "flex", alignItems: "center", gap: 6 }}>
                    <PhoneIncoming size={14} />
                    &nbsp;Phone Call Channel
                  </div>
                  <ul className="blist">
                    <li>International DID number — Brazil +55 or any country via Telnyx</li>
                    <li>24/7 availability — Sofia never sleeps or takes a day off</li>
                    <li>Natural conversation: book, reschedule, cancel, price inquiry</li>
                    <li>Same 15-intent detection as WhatsApp — no degraded experience</li>
                    <li>Human handoff if confidence &lt; 0.50 (same RED zone rule applies)</li>
                    <li>Quiet hours enforced (08h–20h) — no action execution outside window</li>
                    <li>After-call: PIX link + booking confirmation sent automatically via WhatsApp/SMS</li>
                  </ul>
                </div>
                <div className="card card-accent-green">
                  <div className="card-title green" style={{ display: "flex", alignItems: "center", gap: 6 }}>
                    <MessageSquare size={14} />
                    &nbsp;WhatsApp Voice Channel
                  </div>
                  <ul className="blist green">
                    <li>Dedicated WhatsApp Business number provisioned via Twilio</li>
                    <li>Voice note transcription — patient sends a voice message, Sofia replies in text</li>
                    <li>Patients can make a direct WA voice call to Sofia&apos;s number</li>
                    <li>Seamless handoff between voice and text in the same WA thread</li>
                    <li>No separate number to memorise — same WhatsApp contact for everything</li>
                    <li>PIX QR code delivered in-thread immediately after voice booking</li>
                    <li>All TPL_* message templates apply identically post-voice interaction</li>
                  </ul>
                </div>
              </div>

              <h3 className="sub">VAPI Configuration</h3>
              <table className="tbl">
                <thead><tr><th>Parameter</th><th>Value</th><th>Notes</th></tr></thead>
                <tbody>
                  {[
                    ["STT Provider", "Deepgram Nova-2", "Best latency + accuracy for Portuguese (BR)"],
                    ["TTS Provider", "ElevenLabs / Azure Neural", "Sofia voice persona — warm, professional, Brazilian PT"],
                    ["LLM Hook", "n8n webhook (POST)", "Sends transcript → same CLASSIFY → DECIDE pipeline"],
                    ["Interrupt Threshold", "800 ms", "Allows natural conversation interruptions without cut-off"],
                    ["End-of-turn Detection", "Deepgram VAD", "Voice activity detection for clean turn-taking"],
                    ["Silence Timeout", "5 s", "Prompts patient to continue or offers to help"],
                    ["Max Call Duration", "10 min", "Auto-escalates to human after limit — no silent drop"],
                    ["Language", "pt-BR primary", "Auto-detects Spanish / English fallback"],
                  ].map(([c, v, n]) => (
                    <tr key={c}>
                      <td style={{ fontWeight: 600 }}>{c}</td>
                      <td style={{ fontFamily: "monospace", fontSize: 11.5 }}>{v}</td>
                      <td style={{ color: "#78716c" }}>{n}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
              ...
              <div className="g2">
                <div className="card card-accent-amber">
                  <div className="card-title amber" style={{ display: "flex", alignItems: "center", gap: 6 }}>
                    <Shield size={14} />
                    &nbsp;Voice Kill Switches (Sofia_Config)
                  </div>
                  <table className="tbl" style={{ marginBottom: 0 }}>
                    <thead><tr><th>Switch</th><th>Default</th><th>Effect</th></tr></thead>
                    <tbody>
                      {[
                        ["sofia_voice_enabled", "TRUE", "Master toggle — all voice channels"],
                        ["sofia_voice_phone_enabled", "TRUE", "Phone (Telnyx / Twilio) only"],
                        ["sofia_voice_whatsapp_enabled", "TRUE", "WhatsApp voice channel only"],
                        ["sofia_voice_booking_enabled", "TRUE", "Allow bookings over voice"],
                        ["sofia_voice_human_fallback", "TRUE", "Escalate on low confidence"],
                      ].map(([s, d, e]) => (
                        <tr key={s}>
                          <td style={{ fontFamily: "monospace", fontSize: 11 }}>{s}</td>
                          <td><span className={`badge ${d === "TRUE" ? "badge-green" : "badge-slate"}`}>{d}</span></td>
                          <td>{e}</td>
                        </tr>
                      ))}
                    </tbody>
                ...
                <div className="card card-accent-purple">
                  <div className="card-title purple" style={{ display: "flex", alignItems: "center", gap: 6 }}>
                    <Zap size={14} />
                    &nbsp;Voice Cost Estimate
                  </div>
                  <table className="tbl" style={{ marginBottom: 0 }}>
                    <thead><tr><th>Service</th><th>Est. Cost</th><th>Basis</th></tr></thead>
                    <tbody>
                      {[
                        ["VAPI", "~$0.05 / min", "STT + TTS + LLM calls bundled"],
                        ["Telnyx DID", "~$1 / mo / number", "Brazilian +55 DID"],
                        ["Telnyx inbound", "~$0.004 / min", "Ultra-low global rate"],
                        ["Twilio WA number", "~$1 / mo", "WA Business-enabled line"],
                        ["Twilio voice", "~$0.013 / min", "Inbound to WA number"],
                        ["Monthly total", "~$35–70 / mo", "300 pts × 2 min avg, 20% voice"],
                      ].map(([s, c, b]) => (
                        <tr key={s}>
                          <td>{s}</td>
                          <td style={{ fontWeight: 700, color: "#a16207" }}>{c}</td>
                          <td style={{ color: "#78716c", fontSize: 12 }}>{b}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
              ...
              <h3 className="sub">What Sofia Can Do Over Voice</h3>
              <div className="g3">
                {[
                  { icon: <Mic size={20} color="#EC420F" />, title: "Book Appointments", desc: "Full E1→E2 pipeline over voice — exam type, time slot, deposit confirmation, all in one call." },
                  { icon: <Headphones size={20} color="#059669" />, title: "Answer Inquiries", desc: "Exam prep instructions, pricing progression (F1→F4), insurance plans, and result status — all verbally." },
                  { icon: <Server size={20} color="#7c3aed" />, title: "Sync to CRM", desc: "Every voice interaction is logged to Zoho CRM with correlation_id, just like WhatsApp messages." },
                ].map((item) => (
                  <div className="card card-accent" key={item.title}>
                    <div style={{ marginBottom: 8 }}>{item.icon}</div>
                    <div className="card-title blue">{item.title}</div>
                    <p style={{ fontSize: 13, color: "#57534e" }}>{item.desc}</p>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="sec" id="milestones">
              ...
              {([
                ...
                <div key={ph.phase} style={{ marginBottom: 28 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12, paddingBottom: 8, borderBottom: `2px solid ${ph.color}22` }}>
                    <div style={{ width: 10, height: 10, borderRadius: "50%", background: ph.color }} />
                    <span style={{ fontSize: 11, fontWeight: 800, letterSpacing: ".1em", textTransform: "uppercase", color: ph.color }}>{ph.phase}</span>
                  </div>
                  {ph.milestones.map((m) => (
                    <div className="ms-bar" key={m.id} style={{ borderLeft: `3px solid ${m.color}` }}>
                      ...
                      <div className="ms-info">
                        ...
                        {m.note && <div style={{ fontSize: 11, color: m.note.startsWith("↓") ? "#dc2626" : m.note.startsWith("↑") ? "#059669" : m.note.startsWith("⚡") ? "#d97706" : "#0891b2", fontWeight: 600, marginBottom: 2 }}>{m.note}</div>}
              ...
              <h3 className="sub">Price Change Summary</h3>
              <table className="tbl">
            ...
            <div className="sec" id="answers">
              ...
              <h2 className="sec-title">Project Questions &amp; Answers</h2>
              {[
              ...
              <h3 className="sub">Open Questions</h3>
              <table className="tbl">
